...
(function (global, factory) {
    ...
    _createClass(ElementCore, [{
      ...
      set: function set(v) {
        if (Utils$1.isFunction(v)) {
          ...
          this._disableFuncX();
          if (this.hasFlexLayout()) {
            var dx = v - this._layout.originalX;
      ...
      set: function set(v) {
        if (Utils$1.isFunction(v)) {
          ...
          this._disableFuncY();
          if (this.hasFlexLayout()) {
            var dy = v - this._layout.originalY;
   ...
   * @returns
   */
  var TextTokenizer = /*#__PURE__*/function () {
    function TextTokenizer() {
      _classCallCheck(this, TextTokenizer);
    }
    _createClass(TextTokenizer, null, [{
      key: "getTokenizer",
      value:
      ...
      key: "setCustomTokenizer",
      value: function setCustomTokenizer(tokenizer) {
        var _this = this;
        var detectASCII = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
        if (!tokenizer || !detectASCII) {
          this._customTokenizer = tokenizer;
        } else {
          this._customTokenizer = function (text) {
            _newArrowCheck(this, _this);
            return TextTokenizer.containsOnlyASCII(text) ? this.defaultTokenizer(text) : tokenizer(text);
          }.bind(this);
        }
      }
      ...
      key: "containsOnlyASCII",
      value: function containsOnlyASCII(text) {
      ...
      key: "defaultTokenizer",
      value: function defaultTokenizer(text) {
        var words = [];
        var len = text.length;
        var startIndex = 0;
        var i = 0;
  ...
  function wrapText(context, text, wrapWidth, letterSpacing, textIndent, maxLines, suffix, wordBreak) {
    ...
    var tokenize = TextTokenizer$1.getTokenizer();
    var words = tokenize(text)[0].tokens;
    var spaceWidth = measureText(context, " ", letterSpacing);
    var resultLines = [];
    var result = "";
    var spaceLeft = wrapWidth - textIndent;
    var word = "";
    var wordWidth = 0;
    var totalWidth = textIndent;
    var overflow = false;
    for (var j = 0; j < words.length; j++) {
      ...
      if (wordWidth > spaceLeft) {
        ...
        if (j > 0 && word === " ") wordWidth = 0;else result = word;

        // if word is too long, break it (caution: it could produce more than maxLines)
        if (wordBreak && wordWidth > wrapWidth) {
          var broken = breakWord(context, word, wrapWidth, letterSpacing);
          var last = broken.pop();
          for (var _i = 0, _broken = broken; _i < _broken.length; _i++) {
            var k = _broken[_i];
            resultLines.push({
              text: k.text,
              width: k.width
    ...
    if (overflow) {
      var suffixWidth = suffix ? measureText(context, suffix, letterSpacing) : 0;
   ...
   * @param space
   */
  function breakWord(context, word, wordWrapWidth) {
    ...
    for (var i = 0; i < word.length; i++) {
      var c = word.charAt(i);
      token += c;
      var width = measureText(context, token, space);
  ...
  var TextTextureRenderer = /*#__PURE__*/function () {
    function TextTextureRenderer(stage, canvas, settings) {
      ...
      _defineProperty(this, "prevShadowSettings", null);
      this._stage = stage;
      this._canvas = canvas;
      this._context = this._canvas.getContext("2d");
      var dpr = stage.getOption("devicePixelRatio") || 1;
    ...
    _createClass(TextTextureRenderer, [{
      ...
      value: function _load() {
        ...
        if (/*Utils.isWeb &&*/document.fonts) {
          var fontSetting = getFontSetting(this._settings.fontFace, this._settings.fontStyle, this._settings.fontSize, this._stage.getRenderPrecision(), this._stage.getOption("defaultFontFace"));
          try {
      ...
      value: function _calculateRenderInfo() {
        var _linesInfo$r;
        var renderInfo = {};
        var precision = this._stage.getRenderPrecision();
        ...
        var maxLines = this._settings.maxLines;
        ...
        if ((_linesInfo$r = linesInfo.r) !== null && _linesInfo$r !== void 0 && _linesInfo$r.length) {
        ...
        for (var i = 0; i < lines.length; i++) {
          var _width = lines[i].width;
          lineWidths.push(_width);
          maxLineWidth = Math.max(maxLineWidth, _width);
        }
        ...
        var textBaseline = this._settings.textBaseline;
        var verticalAlign = this._settings.verticalAlign;
        var height;
        if (h) {
          height = h;
        } else {
          var baselineOffset = this._settings.textBaseline !== "bottom" ? fontSize * 0.5 : 0;
          height = lineHeight * (lines.length - 1) + baselineOffset + Math.max(lineHeight, fontSize) + (offsetY || 0);
        }

        // calculate vertical draw offset
        if (offsetY === null) {
          if (textBaseline === "top") offsetY = 0;else if (textBaseline === "alphabetic") offsetY = fontSize;else offsetY = fontSize;
        }
        ...
        var rtl = this._settings.rtl;
        var textAlign = this._settings.textAlign;
        if (rtl) {
          if (textAlign === "left") textAlign = "right";else if (textAlign === "right") textAlign = "left";
        }
        var linePositionX;
        var linePositionY;
        var drawLines = [];
        ...
        // Layout lines
        for (var _i = 0, n = lines.length; _i < n; _i++) {
          var lineWidth = lineWidths[_i] || 0;
          ...
          linePositionY = _i * lineHeight + offsetY;
          drawLines.push({
            info: lines[_i],
            x: linePositionX,
            y: linePositionY,
      ...
      value: function _draw() {
        ...
        var precision = renderInfo.precision;
      ...
      key: "_drawLines",
      value: function _drawLines(drawLines, letterSpacing) {
      ...
      key: "_fillTextWithLetterSpacing",
      value: function _fillTextWithLetterSpacing(ctx, text, x, y, letterSpacing) {
      ...
      key: "_restoreShadow",
      value: function _restoreShadow() {
        var settings = this.prevShadowSettings;
        var ctx = this._context;
      ...
      key: "_drawShadow",
      value: function _drawShadow(precision) {
        var ctx = this._context;
        this.prevShadowSettings = [ctx.shadowColor, ctx.shadowOffsetX, ctx.shadowOffsetY, ctx.shadowBlur];
      ...
      key: "_drawHighlight",
      value: function _drawHighlight(precision, renderInfo) {
        ...
        for (var i = 0; i < renderInfo.lines.length; i++) {
          var drawLine = renderInfo.lines[i];
      ...
      key: "measureLines",
      value: function measureLines(lines) {
        var _this3 = this;
      ...
      key: "wrapText",
      value: function wrapText$1(text, wordWrapWidth) {
        var lines = text.split(/(?:\r\n|\r|\n)/);
        var renderLines = [];
        var maxLines = this._settings.maxLines;
        var _getSuffix = getSuffix(this._settings.maxLinesSuffix, this._settings.textOverflow, this._settings.wordWrap),
          suffix = _getSuffix.suffix,
          nowrap = _getSuffix.nowrap;
        var wordBreak = this._settings.wordBreak;
        for (var i = 0; i < lines.length; i++) {
          var line = lines[i];
          var tempLines = wrapText(this._context, line, wordWrapWidth, this._settings.letterSpacing, i === 0 ? this._settings.textIndent : 0, nowrap ? 1 : maxLines, suffix, wordBreak);
          if (maxLines === 0) {
            // add all
            renderLines.push.apply(renderLines, _toConsumableArray(tempLines));
          } else {
            // add up to
            ...
            if (maxLines === 0) {
              if (i < lines.length - 1) {
                var lastLine = renderLines[renderLines.length - 1];
  ...
  function extractTags(source) {
    var tags = ["<i>", "</i>", "<b>", "</b>", "</color>"];
    var reTag = /<i>|<\/i>|<b>|<\/b>|<color=0[xX][0-9a-fA-F]{6,8}>|<\/color>/g;
    var output = "";
    var m;
    var lastIndex = 0;
    while (m = reTag.exec(source)) {
      output += source.substring(lastIndex, m.index);
      var code = TAG_DEFAULTS[m[0]];
      if (code === undefined) {
        code = 0x2460 + tags.length;
        tags.push(m[0]);
      }
      output += "\u200B".concat(String.fromCharCode(code), "\u200B");
  ...
  function measureLine(line) {
    var _line$words,
      _this = this;
    if (((_line$words = line.words[line.words.length - 1]) === null || _line$words === void 0 ? void 0 : _line$words.text) === " ") {
      line.words.pop();
    }
    line.words.forEach(function (token) {
      _newArrowCheck(this, _this);
      return line.width += token.width;
    }.bind(this));
  }
  ...
  function createLineStyle(tags, baseFont, color) {
    var _this2 = this;
    var isStyled = tags.length > 0;
    var bold = 0;
    var italic = 0;
    var colors = [StageUtils.getRgbaString(color)];
    var updateStyle = function updateStyle(code) {
      _newArrowCheck(this, _this2);
      var tag = tags[code - 0x2460];
      if (!tag) return false;
      if (tag === "<b>") {
        bold++;
      } else if (tag === "</b>") {
        ...
        var hex = tag.substring(9, tag.length - 1);
  ...
  function layoutSpans(ctx, spans, lineStyle, wrapWidth, textIndent, maxLines, suffix, wordBreak, letterSpacing, allowTruncation) {
    var _spans$,
      _this3 = this;
    // styling
    var isStyled = lineStyle.isStyled,
      baseStyle = lineStyle.baseStyle,
      updateStyle = lineStyle.updateStyle,
      getStyle = lineStyle.getStyle;
    var initialStyle = getStyle();
    ctx.font = initialStyle.font;
    var style = isStyled ? initialStyle : undefined;

    // cached metrics
    var spaceWidth = measureText(ctx, " ", letterSpacing);
    var suffixWidth = measureText(ctx, suffix, letterSpacing);

    // layout state
    var rtl = Boolean((_spans$ = spans[0]) === null || _spans$ === void 0 ? void 0 : _spans$.rtl);
    var primaryRtl = rtl;
    var line = {
      rtl: rtl,
      width: textIndent,
    ...
    // concatenate words
    var appendWords = function appendWords() {
      var _line$words3;
      _newArrowCheck(this, _this3);
      if (rtl !== primaryRtl) {
        words.reverse();
      ...
      if (line.words.length > 1) {
        var _words$, _line$words2;
        if (((_words$ = words[0]) === null || _words$ === void 0 ? void 0 : _words$.text) === " " && ((_line$words2 = line.words[line.words.length - 1]) === null || _line$words2 === void 0 ? void 0 : _line$words2.text) === " ") {
          words.shift();
        }
      }
      (_line$words3 = line.words).push.apply(_line$words3, _toConsumableArray(words));
      words = [];
    }.bind(this);
    ...
    for (var si = 0; si < spans.length; si++) {
      ...
      for (var ti = 0; ti < tokens.length; ti++) {
        ...
        // measure word
        var width = isSpace ? spaceWidth : measureText(ctx, text, letterSpacing);
        x += width;
        ...
        if (x > wrapWidth) {
          ...
          if (width > wrapWidth) {
            ...
            if (wordBreak) {
              var broken = breakWord(ctx, text, wrapWidth, 0);
              var last = broken.pop();
    ...
    if (overflow) {
      ...
      var maxLineWidth = wrapWidth - suffixWidth;
      if (line.width > maxLineWidth) {
        ...
        var lastIndex = line.words.length - 1;
        var word = line.words[lastIndex];
        var index = lastIndex;

        // TODO: this works well for English but not for embedded RTL
        if (primaryRtl && !word.rtl) {
          var removeOppositeEnd = true;
        ...
        while (line.width > maxLineWidth) {
          var _last = line.words.pop();
          line.width -= _last.width;
          var maxWidth = maxLineWidth - line.width;
          if (allowTruncation && maxWidth > 0) {
            ...
            var reversed = primaryRtl !== _rtl;
            do {
              _text = reversed ? trimWordStart(_text, _rtl) : trimWordEnd(_text, _rtl);
              _width = measureText(ctx, _text, letterSpacing);
            } while (_width > maxWidth);
            if (_width > suffixWidth) {
              _last = _objectSpread2(_objectSpread2({}, _last), {}, {
                text: _text,
                width: _width
              });
              line.words.push(_last);
              line.width += _width;
              break;
            }
          }
  ...
  function trimRtlWordEnd(text) {
    var match = text.match(rePunctuationStart);
    if (match) {
    ...
    if (match) {
      var _punctuation = match[0];
      text = text.substring(0, text.length - _punctuation.length);
      if (text.length > 0) {
        return text.substring(0, text.length - 1) + _punctuation;
      } else {
        return _punctuation.substring(1);
      }
    }
    return text.substring(0, text.length - 1);
  }
  ...
  var TextTextureRendererAdvanced = /*#__PURE__*/function (_TextTextureRenderer) {
    _inherits(TextTextureRendererAdvanced, _TextTextureRenderer);
    var _super = _createSuper(TextTextureRendererAdvanced);
    function TextTextureRendererAdvanced() {
    ...
    _createClass(TextTextureRendererAdvanced, [{
      ...
      value: function wrapText(text, wordWrapWidth) {
        var styled = this._settings.advancedRenderer;

        // styled renderer' base font should not include styling
        var baseFont = getFontSetting(this._settings.fontFace, styled ? "" : this._settings.fontStyle, this._settings.fontSize, this._stage.getRenderPrecision(), this._stage.getOption("defaultFontFace"));
        var _getSuffix = getSuffix(this._settings.maxLinesSuffix, this._settings.textOverflow, this._settings.wordWrap),
          suffix = _getSuffix.suffix,
          nowrap = _getSuffix.nowrap;
        var wordBreak = this._settings.wordBreak;
        var letterSpacing = this._settings.letterSpacing;
        var allowTextTruncation = TextTexture.allowTextTruncation;
        var tags;
        if (styled) {
          var extract = extractTags(text);
          tags = extract.tags;
          text = extract.output;
        } else {
          tags = [];
        }
        var lineStyle = createLineStyle(tags, baseFont, this._settings.textColor);
        var tokenize = TextTokenizer$1.getTokenizer();
        var sourceLines = text.split(/[\r\n]/g);
        var wrappedLines = [];
        var remainingLines = this._settings.maxLines;
        for (var i = 0; i < sourceLines.length; i++) {
          var line = sourceLines[i];
          var spans = tokenize(line);
          var lines = layoutSpans(this._context, spans, lineStyle, wordWrapWidth, i === 0 ? this._settings.textIndent : 0, nowrap ? 1 : remainingLines, suffix, wordBreak, letterSpacing, allowTextTruncation);
          wrappedLines.push.apply(wrappedLines, _toConsumableArray(lines));
          if (remainingLines > 0) {
      ...
      value: function _drawLines(drawLines, letterSpacing) {
        ...
        var currentStyle;
        for (var i = 0, n = drawLines.length; i < n; i++) {
          var _drawLine$info$words;
          var drawLine = drawLines[i];
          var words = (_drawLine$info$words = drawLine.info.words) !== null && _drawLine$info$words !== void 0 ? _drawLine$info$words : [];
          var y = drawLine.y;
          var x = drawLine.x;
          for (var j = 0; j < words.length; j++) {
            var _ref = words[j],
    ...
    _createClass(TextTexture, [{
      ...
      value: function _getSourceLoader() {
        ...
        return function (cb) {
          ...
          var renderer = TextTexture.renderer(this.stage, canvas, args);
          var p = renderer.draw();
          var texParams = {};
      ...
      value: function draw(operation, target) {
        ...
        for (var i = 0; i < length; i++) {
          ...
          if (rect) {
            ...
            var colorize = !white;
            var drawSourceX = sourceX;
            var drawSourceW = sourceW;
            var destX = 0;
            var drawSourceY = sourceY;
            var drawSourceH = sourceH;
            var destY = 0;
            var mirroring = sourceW < 0 || sourceH < 0;
            if (mirroring) {
              ctx.save();
              if (sourceW < 0) {
      ...
      value: function _setColorGradient(ctx, vc) {
        ...
        if (vc._colorUl === vc._colorUr) {
          if (vc._colorBl === vc._colorBr) {
            if (vc._colorUl === vc._colorBl) ; else {
  ...
  var WebPlatform = /*#__PURE__*/function () {
    ...
    _createClass(WebPlatform, [{
      ...
      key: "switchLoop",
      value: function switchLoop(hasChanges) {
        var _this = this;
        if (hasChanges) {
          this._idleLoopCounter = 0;
          return false;
        ...
        this._loopHandler = setInterval(function () {
          ...
          var hasChanges = this.stage.renderFrame();
          if (hasChanges) {
            clearInterval(this._loopHandler);
            this.startLoop();
          }
      ...
      value: function loop() {
        var _this2 = this;
        var pauseRafLoopOnIdle = this.stage.getOption("pauseRafLoopOnIdle");
        var _lp = function lp() {
          _newArrowCheck(this, _this2);
          this._awaitingLoop = false;
          if (this._looping) {
  ...
  var Application = /*#__PURE__*/function (_Component) {
    ...
    _createClass(Application, [{
      ...
      value: function focusTopDownEvent(events) {
        ...
        var n = path.length;
        var _this$getDirectionAwa = this.getDirectionAwareEvents(events),
          eventsLtr = _this$getDirectionAwa.eventsLtr,
          eventsRtl = _this$getDirectionAwa.eventsRtl,
          isHorizontalDirection = _this$getDirectionAwa.isHorizontalDirection;
        for (var _len = arguments.length, args = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
          args[_key - 1] = arguments[_key];
        }
        for (var i = 0; i < n; i++) {
      ...
      value: function focusBottomUpEvent(events) {
        var path = this.focusPath;
        var n = path.length;
        var _this$getDirectionAwa2 = this.getDirectionAwareEvents(events),
          eventsLtr = _this$getDirectionAwa2.eventsLtr,
          eventsRtl = _this$getDirectionAwa2.eventsRtl,
          isHorizontalDirection = _this$getDirectionAwa2.isHorizontalDirection;
        for (var _len2 = arguments.length, args = new Array(_len2 > 1 ? _len2 - 1 : 0), _key2 = 1; _key2 < _len2; _key2++) {
          args[_key2 - 1] = arguments[_key2];
        }
        for (var i = n - 1; i >= 0; i--) {
      ...
      value: function fireBottomUpClickHandler(obj) {
        ...
        while (child !== null) {
          if (child && child["_handleClick"]) {
            var _child$core$_worldCon = child.core._worldContext,
              px = _child$core$_worldCon.px,
              py = _child$core$_worldCon.py;
            var cx = px * precision;
            var cy = py * precision;
            var localCoords = {
              x: clientX - cx,
  ...
  var ListComponent = /*#__PURE__*/function (_Component) {
    ...
    function ListComponent(stage) {
      ...
      _this = _super.call(this, stage);
      _this._wrapper = _get((_assertThisInitialized(_this), _getPrototypeOf(ListComponent.prototype)), "_children", _assertThisInitialized(_this)).a({});
  ...
  exports.Base = Base;
  exports.BloomComponent = BloomComponent;
  exports.BorderComponent = BorderComponent;
  exports.BoxBlurShader = BoxBlurShader;
  exports.C2dBlurShader = BlurShader;
  exports.C2dDefaultShader = DefaultShader;
  exports.C2dGrayscaleShader = C2dGrayscaleShader;
  exports.C2dShader = C2dShader;
  exports.CircularPushShader = CircularPushShader;
  exports.Component = Component;
  exports.DitheringShader = DitheringShader;
  exports.Element = Element;
  exports.ElementCore = ElementCore;
  exports.ElementTexturizer = ElementTexturizer;
  exports.EventEmitter = EventEmitter;
  exports.FadeOutShader = FadeOutShader;
  exports.FastBlurComponent = FastBlurComponent;
  exports.HoleShader = HoleShader;
  exports.HtmlTexture = HtmlTexture;
  exports.ImageTexture = ImageTexture;
  exports.InversionShader = InversionShader;
  exports.Light3dShader = Light3dShader;
  exports.LinearBlurShader = LinearBlurShader;
  exports.ListComponent = ListComponent;
  exports.MagnifierShader = MagnifierShader;
  exports.NoiseTexture = NoiseTexture;
  exports.ObjMerger = ObjMerger;
  exports.ObjectListProxy = ObjectListProxy;
  exports.ObjectListWrapper = ObjectListWrapper;
  exports.OutlineShader = OutlineShader;
  exports.PerspectiveShader = PerspectiveShader;
  exports.PixelateShader = PixelateShader;
  exports.RadialFilterShader = RadialFilterShader;
  exports.RadialGradientShader = RadialGradientShader;
  exports.RectangleTexture = RectangleTexture;
  exports.RoundedRectangleShader = RoundedRectangleShader;
  exports.SmoothScaleComponent = SmoothScaleComponent;
  exports.SourceTexture = SourceTexture;
  exports.SpinnerShader = SpinnerShader;
  exports.SpinnerShader2 = SpinnerShader2;
  exports.Stage = Stage;
  exports.StageUtils = StageUtils;
  exports.StaticCanvasTexture = StaticCanvasTexture;
  exports.StaticTexture = StaticTexture;
  exports.TextTexture = TextTexture;
  exports.TextTextureRenderer = TextTextureRenderer;
  exports.TextTextureRendererAdvanced = TextTextureRendererAdvanced;
  exports.TextTokenizer = TextTokenizer$1;
  exports.Texture = Texture;
  exports.Tools = Tools;
  exports.Utils = Utils$1;
  exports.VignetteShader = VignetteShader;
  exports.WebGLDefaultShader = DefaultShader$1;
